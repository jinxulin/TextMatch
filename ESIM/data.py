...
class LCQMC_Dataset(Dataset):
    def __init__(self, LCQMC_file, word2idx, max_char_len):
        p, h, self.label = load_sentences(LCQMC_file)
        self.p_list, self.p_lengths, self.h_list, self.h_lengths = word_index(
            p, h, word2idx, max_char_len)
...
def word_index(p_sentences, h_sentences, word2idx, max_char_len):
    ...
    for p_sentence, h_sentence in zip(p_sentences, h_sentences):
        p = [word2idx.get(word, word2idx['<unk>']) for word in p_sentence]
        h = [word2idx.get(word, word2idx['<unk>']) for word in h_sentence]
        p_list.append(p)
...
def load_embeddings(embdding_path):
    model = gensim.models.KeyedVectors.load_word2vec_format(
        embdding_path, binary=False)
    model.add_vector('<unk>', np.mean(model.vectors, 0))
    embedding_matrix = np.zeros(
        (len(model.index_to_key) + 1, model.vector_size))
    # 填充向量矩阵
